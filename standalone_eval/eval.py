...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.3, 0.7, 5)):
...
def compute_mr_r5(submission, ground_truth, iou_thds=np.linspace(0.3, 0.7, 5)):
    """If a predicted segment has IoU >= iou_thd with one of the 1st GT segment, we define it positive"""
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    pred_qid2window_top5 = {d["qid"]: d["pred_relevant_windows"][:5] for d in submission}  # :2 rm scores
    for k, v in pred_qid2window_top5.items():
        pred_qid2window_top5[k] = [_v[:2] for _v in v]
    ...
    for d in ground_truth:
        ...
        if len(cur_gt_windows) > 0:  # select the GT window that has the highest IoU
            cur_ious = compute_temporal_iou_batch_cross(
                np.array(pred_qid2window_top5[cur_qid]), np.array(d["relevant_windows"]),
            )[0]
            cur_max_iou_idx = np.argmax(cur_ious)
            pred_qid2window_top5[cur_qid] = pred_qid2window_top5[cur_qid][cur_max_iou_idx]
        gt_qid2window[cur_qid] = cur_gt_windows[0]

    qids = list(pred_qid2window_top5.keys())
    pred_windows = np.array([pred_qid2window_top5[k] for k in qids]).astype(float)
    gt_windows = np.array([gt_qid2window[k] for k in qids]).astype(float)
    pred_gt_iou = compute_temporal_iou_batch_paired(pred_windows, gt_windows)
...
def preprocessing(window, opt):
    if opt is None:
        return window

    max_length = opt.max_v_l * opt.clip_length
    return [min(window[0], max_length), min(window[1], max_length)]

def get_data_by_range(submission, ground_truth, len_range, opt):
    ...
    for d in ground_truth:
        d['relevant_windows'] = [preprocessing(w, opt) for w in d["relevant_windows"]]
...
def eval_moment_retrieval(submission, ground_truth, opt, verbose=True):
    length_ranges = [[0, float('inf')]]
    range_names = ["full"]

    if opt.dset_name in ['hl', 'charades']:
        length_ranges = [[0, 150]]
    ...
    for l_range, name in zip(length_ranges, range_names):
        if verbose:
            start_time = time.time()
        _submission, _ground_truth = get_data_by_range(submission, ground_truth, l_range, opt)
...
def eval_submission(submission, ground_truth, opt, verbose=True, match_number=True):
    ...
    if "pred_saliency_scores" in submission[0] and opt.dset_name == 'hl':
...
def eval_main():
    ...
    results = eval_submission(submission, gt, args, verbose=verbose)
    if verbose:
