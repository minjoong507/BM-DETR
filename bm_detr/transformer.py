...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=10, num_encoder_layers=6,
        ...
        self.encoder = TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm, d_model)
    ...
    def forward(self, src, mask, refpoint_embed, pos_embed, vid_length=None):
        ...
        if self.num_patterns == 0:
            tgt = self.queries.weight.unsqueeze(1).repeat(1, bs, 1) if self.queries else torch.zeros(num_queries, bs, self.d_model, device=refpoint_embed.device)
        ...
        # only vid
        # memory, mask, pos_embed = memory[:vid_length], mask[:, :vid_length], pos_embed[:,:vid_length]
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.modulate_hw_attn:
                reft_cond = self.ref_anchor_head(output).sigmoid()  # nq, bs, 2
...
class AttentivePooling(nn.Module): # borrow from LGI (https://arxiv.org/abs/2004.07514).
    def __init__(self, hidden_dim):
        ...
        if self.use_embedding:
            self.fc = nn.Linear(self.feat_dim, hidden_dim)
