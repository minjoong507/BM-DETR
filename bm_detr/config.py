...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--dset_name", type=str,
                            choices=["hl", "charades", "charades-CD", "activitynet", "tacos"])
        ...
        parser.add_argument("--lr_warmup", type=float, default=-1, help="")
        ...
        parser.add_argument("--eval_epoch_interval", type=int, default=1,
                            help="interval epoch of performing evaluation process")
        ...
        parser.add_argument("--v_feat_type", type=str, help="text/query type of feature")
        parser.add_argument("--t_feat_type", type=str, help="text/query type of feature")
        ...
        # Model config
        parser.add_argument('--position_embedding', default='sine', type=str, choices=('sine', 'learned', 'trainable'),
        ...
        parser.add_argument('--num_queries', default=10, type=int, help="Number of learnable span")
        ...
        parser.add_argument("--use_temporal_shifting", action="store_true",
                            help="Randomly move gt moment in videos.")

        # other model configs
        parser.add_argument("--n_input_proj", type=int, default=2, help="#layers to encoder input")
        parser.add_argument("--contrastive_hdim", type=int, default=256, help="dim for contrastive embeddings")
        parser.add_argument("--temperature", type=float, default=0.07, help="temperature nce contrastive_align_loss")

        # Loss
        parser.add_argument("--additional_losses", default=[], type=list)
        ...
        parser.add_argument("--lw_prob_loss_coef", type=float, default=2.)
        parser.add_argument('--lw_contrastive_loss_coef', default=1., type=float,
                            help="weight for contrastive loss, set to 0 will ignore")

        parser.add_argument("--no_sort_results", action="store_true",
                            help="do not sort results, use thifs for moment query visualization")
        parser.add_argument("--max_before_nms", type=int, default=10)
        parser.add_argument("--max_after_nms", type=int, default=10)
        parser.add_argument("--stop_metric", type=str, default="MR-full-R1@0.7",
                            choices=["MR-full-R1@0.5", "MR-full-R1@0.7", "MR-full-mAP"])
        parser.add_argument("--conf_thd", type=float, default=0.0, help="only keep windows with conf >= conf_thd")
        parser.add_argument("--prob_thd", type=float, default=1.0, help="")
    ...
    def parse(self):
        ...
        if isinstance(self, TestOptions):
            ...
            for arg in saved_options:  # use saved options to overwrite all BaseOptions args.
                if arg not in ["results_root", "num_workers", "nms_thd", "debug", "max_pred_l", "min_pred_l",
                               "resume", "resume_all", "no_sort_results", "eval_split_name", "eval_path"]:
